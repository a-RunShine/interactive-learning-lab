...
**执行流程：**
1. Agent 遍历意图关键词 → 工具关键词，逐一匹配用户诉求
2. 命中 → 静默使用该领域，直接执行 init（用户无感知）
3. 全部未命中 → 提示用户选择：`[编程语言 | 工具与平台 | 数据与算法 | 架构与设计 | 工作流与方法论 | 其他]`
4. 用户在协商阶段明确说"属于XX领域" → 直接使用，跳过推断
...
# 从 stdout JSON 中提取 domain 字段，构建 session 记录
...
- 用户说"看仪表盘"或"学习记录"时，先执行上条命令刷新数据，再引导打开 `dashboard.html`
- 后续打开 `dashboard.html` 查看仪表盘
